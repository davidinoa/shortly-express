...
describe('', function() {
  ...
  xdescribe('Sessions and cookies', function() {
    ...
    it('saves a new session when the server receives a request', function(done) {
      requestWithSession('http://127.0.0.1:3001/', function(err, res, body) {
        if (err) { return done(err); }
        var queryString = 'SELECT * FROM sessions';
    ...
    it('sets and stores a cookie on the client', function(done) {
      requestWithSession('http://127.0.0.1:3001/', function(error, res, body) {
        if (error) { return done(error); }
        var cookies = cookieJar.getCookies('http://127.0.0.1:3001/');
    ...
    it('destroys session and cookie when logs out', function(done) {
      addUser(function(err, res, body) {
        ...
        requestWithSession('http://127.0.0.1:3001/logout', function(error, response, resBody) {
          if (error) { return done(error); }
  ...
  xdescribe('Privileged Access:', function() {
    ...
    it('Redirects to login page if a user tries to access the main page and is not signed in', function(done) {
      request('http://127.0.0.1:3001/', function(error, res, body) {
        if (error) { return done(error); }
    ...
    it('Redirects to login page if a user tries to create a link and is not signed in', function(done) {
      request('http://127.0.0.1:3001/create', function(error, res, body) {
        if (error) { return done(error); }
        expect(res.req.path).to.equal('/login');
        done();
      });
    ...
    it('Redirects to login page if a user tries to see all of the links and is not signed in', function(done) {
      request('http://127.0.0.1:3001/links', function(error, res, body) {
        if (error) { return done(error); }
        expect(res.req.path).to.equal('/login');
        done();
      });
  ...
  xdescribe('Link creation:', function() {
    ...
    afterEach(function(done) {
      requestWithSession('http://127.0.0.1:3001/logout', done);
    });
    ...
    describe('With previously saved urls:', function() {
      ...
      it('Shortcode redirects to correct url', function(done) {
        var options = {
          'method': 'GET',
          'uri': 'http://127.0.0.1:3001/' + link.code
        };
      ...
      it('Shortcode redirects to index if shortcode does not exist', function(done) {
        var options = {
          'method': 'GET',
          'uri': 'http://127.0.0.1:3001/doesNotExist'
        };

        requestWithSession(options, function(error, res, body) {
          if (error) { return done(error); }
          var currentLocation = res.request.href;
          expect(currentLocation).to.equal('http://127.0.0.1:3001/');
          done();
        });
      });
